...
type Promoter struct {
	...
	// compressor decodes the buffer's content encodings. It owns pooled
	// zstd state, so it is built once and shared across ticks.
...
func (p *Promoter) loadCandidate(ctx context.Context, network string, cand *candidate, rows []*persistence.BeaconBlock) bool {
	...
	for _, row := range rows {
		...
		raw, err := p.decompress(*data, row.ContentEncoding)
		if err != nil {
			malformed = true
...
func (p *Promoter) promote(ctx context.Context, pass *epochPass, cand *candidate, parent parentInfo, triggers []string, branch string) error {
	...
	if _, err := p.corpus.SaveRaw(ctx, &store.SaveParams{Data: bytes.NewReader(cand.raw), Location: capturePath(network, forkName, id, captureBlockName)}); err != nil {
	...
	if _, err := p.corpus.SaveRaw(ctx, &store.SaveParams{Data: bytes.NewReader(data), Location: manifestKey}); err != nil {
...
func (p *Promoter) writeState(ctx context.Context, network string, raw []byte, sha string) error {
	...
	if _, err := p.corpus.SaveRaw(ctx, &store.SaveParams{Data: bytes.NewReader(raw), Location: location}); err != nil {
		return err
	}
...
// decompress undoes the buffer's content encoding, whatever the capture
// pipeline happened to write: zstd today, gzip for objects stored before the
// move, identity for the rest. Corpus objects are always written
// uncompressed: the sha256 of the raw SSZ is the identity.
func (p *Promoter) decompress(data []byte, contentEncoding string) ([]byte, error) {
	// An absent Content-Encoding is stored, not encoded; the compression
	// package names that "identity".
	if contentEncoding == "" {
		return data, nil
	}

	algorithm, err := compression.GetCompressionAlgorithmFromContentEncoding(contentEncoding)
	if err != nil {
		return nil, err
	}
	...
	reader, err := p.compressor.NewReader(algorithm, bytes.NewReader(data))
	if err != nil {
		return nil, err
	}
