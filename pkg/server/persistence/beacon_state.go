...
type BeaconState struct {
	...
	Slot                 int64 `gorm:"index:idx_beacon_state_slot,where:deleted_at IS NULL;index:idx_beacon_state_node_slot_stateroot_network_fetchedat_deletedat,priority:2"`
	Epoch                int64
	StateRoot            string    `gorm:"index;index:idx_beacon_state_node_slot_stateroot_network_fetchedat_deletedat,priority:3"`
	FetchedAt            time.Time `gorm:"index:idx_beacon_state_node_slot_stateroot_network_fetchedat_deletedat,priority:5;index:idx_beacon_state_fetchedat_network_deletedat,where:deleted_at IS NULL,priority:1"`
	BeaconImplementation string
	NodeVersion          string         `gorm:"not null;default:''"`
	Location             string         `gorm:"not null;default:''"`
	Network              string         `gorm:"not null;default:'';index:idx_beacon_state_node_slot_stateroot_network_fetchedat_deletedat,priority:4;index:idx_beacon_state_network,where:deleted_at IS NULL"`
	DeletedAt            gorm.DeletedAt `gorm:"index:idx_beacon_state_node_slot_stateroot_network_fetchedat_deletedat,priority:6"`
}
