...
import { useBeaconBadBlocks } from '@hooks/useQuery';

export default function BeaconBadBlockId({ id }: { id: string }) {
  ...
  const handleSearch = (key: string, value: unknown) => {
    setValue(key, value);
    setLocation('/beacon_bad_block');
  };

  let errorMessage = undefined;

  if (error) {
    errorMessage = 'Error fetching beacon bad block';
    if (error instanceof Error) {
      errorMessage = `Error fetching beacon bad block: ${error.message}`;
    }
  } else if (!isLoading && !block) {
    errorMessage = 'Beacon bad block not found';
  }
        ...
        <dl className="sm:divide-y sm:divide-gray-200 dark:divide-gray-900">
          ...
          <div className="py-4 sm:py-5  px-4 sm:grid sm:grid-cols-5 sm:gap-4 sm:px-6">
            ...
            <dd className="mt-1 text-sm text-sky-500 font-bold sm:mt-0 sm:col-span-4 flex">
              <span
                ...
                onClick={() => handleSearch('beaconBadBlockNode', block?.node)}
              >
                {isLoading ? (
          ...
          <div className="py-4 sm:py-5  px-4 sm:grid sm:grid-cols-5 sm:gap-4 sm:px-6">
            ...
            <dd className="mt-1 text-sm text-sky-500 font-bold sm:mt-0 sm:col-span-4 flex">
              <span
                className="relative top-1 group transition cursor-pointer"
                onClick={() =>
                  handleSearch('beaconBadBlockNodeImplementation', block?.beacon_implementation)
                }
              >
                {isLoading ? (
                  <div className="h-5 w-16 bg-gray-600/35 rounded-xl animate-pulse"></div>
          ...
          <div className="py-4 sm:py-5  px-4 sm:grid sm:grid-cols-5 sm:gap-4 sm:px-6">
            ...
            <dd className="mt-1 text-sm text-sky-500 font-bold sm:mt-0 sm:col-span-4 flex">
              <span
                className="relative top-1 group transition cursor-pointer"
                onClick={() => handleSearch('beaconBadBlockNodeVersion', block?.node_version)}
              >
                {isLoading ? (
          ...
          <div className="py-4 sm:py-5  px-4 sm:grid sm:grid-cols-5 sm:gap-4 sm:px-6">
            ...
            <dd className="mt-1 text-sm text-sky-500 font-bold sm:mt-0 sm:col-span-4 flex">
              <span
                className="relative top-1 group transition cursor-pointer"
                onClick={() => handleSearch('beaconBadBlockEpoch', block?.epoch.toString())}
              >
                {isLoading ? (
          ...
          <div className="py-4 sm:py-5  px-4 sm:grid sm:grid-cols-5 sm:gap-4 sm:px-6">
            ...
            <dd className="mt-1 text-sm text-sky-500 font-bold sm:mt-0 sm:col-span-4 flex">
              <span
                className="relative top-1 group transition cursor-pointer"
                onClick={() => handleSearch('beaconBadBlockSlot', block?.slot.toString())}
              >
                {isLoading ? (
            ...
            <dd className="mt-1 text-sm text-sky-500 font-bold sm:mt-0 sm:col-span-4">
              <span className="lg:hidden font-mono flex">
                <span
                  ...
                  onClick={() => handleSearch('beaconBadBlockBlockRoot', block?.block_root)}
                >
                  {isLoading ? (
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
                  ...
                  {block?.content_hash && (
                    <span className="flex font-mono">
                      <span className="relative top-1">{block.content_hash}</span>
                      <CopyToClipboard text={block.content_hash} className="ml-2" />
                    </span>
                  )}
                </div>
              )}
