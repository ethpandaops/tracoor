...
import BeaconBadBlockId from '@components/BeaconBadBlockId';
import Pagination from '@components/Pagination';
import useNetwork from '@contexts/network';
import { Selection } from '@contexts/selection';
import { useBeaconBadBlocks, useBeaconBadBlocksCount } from '@hooks/useQuery';
...
export default function BeaconBadBlockTable({ id }: { id?: string }) {
  ...
  const { data, isLoading, error } = useBeaconBadBlocks({
    network: network ? network : undefined,
    slot: beaconBadBlockSlot ? parseInt(beaconBadBlockSlot) : undefined,
    epoch: beaconBadBlockEpoch ? parseInt(beaconBadBlockEpoch) : undefined,
    block_root: beaconBadBlockBlockRoot ? beaconBadBlockBlockRoot : undefined,
    node: beaconBadBlockNode ? beaconBadBlockNode : undefined,
    node_version: beaconBadBlockNodeVersion ? beaconBadBlockNodeVersion : undefined,
    beacon_implementation: beaconBadBlockNodeImplementation
      ? beaconBadBlockNodeImplementation
      : undefined,
    pagination: {
      limit: itemsPerPage,
  ...
  const { data: count } = useBeaconBadBlocksCount({
    network: network ? network : undefined,
    slot: beaconBadBlockSlot ? parseInt(beaconBadBlockSlot) : undefined,
    epoch: beaconBadBlockEpoch ? parseInt(beaconBadBlockEpoch) : undefined,
    block_root: beaconBadBlockBlockRoot ? beaconBadBlockBlockRoot : undefined,
  ...
  return (
    <>
              ...
              <thead className="bg-sky-400">
                <tr className="divide-x divide-orange-300">
                  ...
                  <th
                    scope="col"
                    className={classNames(
                      sortConfig.key !== 'block_root' ? 'cursor-s-resize' : '',
                      sortConfig.key === 'block_root' && sortConfig.direction === 'DESC'
                        ? 'cursor-n-resize'
                        : '',
                      sortConfig.key === 'block_root' && sortConfig.direction === 'ASC'
                        ? 'cursor-s-resize'
                        : '',
                      'py-3.5 pl-4 pr-4 text-left text-sm font-semibold text-gray-50 hidden 2xl:table-cell',
                    )}
                    onClick={() => handleSort('block_root')}
                  >
                    <div className="flex">
                      <span className="whitespace-nowrap">Block root</span>
                      <span>
                        {sortConfig.key === 'block_root' ? (
                          sortConfig.direction === 'DESC' ? (
                            <ArrowDownIcon className="ml-2 h-5 w-5" />
                          ) : (
                            <ArrowUpIcon className="ml-2 h-5 w-5" />
                      ...
                      <tr key={row.id} className="divide-x divide-orange-300">
                        ...
                        <td className="whitespace-nowrap py-4 pl-4 pr-4 text-sm font-bold text-gray-600">
                          <div className=" w-fit">
                            <span
                              ...
                              onClick={() => setValue('beaconBadBlockNode', row.node)}
                            >
                              {row.node}
                        ...
                        <td className="whitespace-nowrap py-4 pl-4 pr-4 text-sm font-bold text-gray-600 hidden 4xl:table-cell">
                          <div className=" w-fit">
                            <span
                              className="relative top-1 group transition cursor-pointer"
                              onClick={() =>
                                setValue('beaconBadBlockNodeVersion', row.node_version)
                              }
                            >
                              {row.node_version}
                              <span className="relative -top-0.5 block max-w-0 group-hover:max-w-full transition-all duration-500 h-0.5 bg-sky-400"></span>
                        ...
                        <td className="whitespace-nowrap py-4 pl-4 pr-4 text-sm font-bold text-gray-600 hidden 3xl:table-cell">
                          <div className=" w-fit">
                            <span
                              className="relative top-1 group transition cursor-pointer"
                              onClick={() => setValue('beaconBadBlockEpoch', row.epoch)}
                            >
                              {row.epoch}
                              <span className="relative -top-0.5 block max-w-0 group-hover:max-w-full transition-all duration-500 h-0.5 bg-sky-400"></span>
                            </span>
                        ...
                        <td className="whitespace-nowrap py-4 pl-4 pr-4 text-sm font-bold text-gray-600">
                          <div className=" w-fit">
                            <span
                              className="relative top-1 group transition cursor-pointer"
                              onClick={() => setValue('beaconBadBlockSlot', row.slot)}
                            >
                              {row.slot}
                              <span className="relative -top-0.5 block max-w-0 group-hover:max-w-full transition-all duration-500 h-0.5 bg-sky-400"></span>
                            </span>
                        ...
                        <td className="whitespace-nowrap py-4 pl-4 pr-4 text-sm font-bold text-gray-600 hidden 2xl:table-cell">
                          <div className=" w-fit">
                            <span
                              ...
                              onClick={() => setValue('beaconBadBlockBlockRoot', row.block_root)}
                            >
                              {row.block_root}
                              <span className="relative -top-0.5 block max-w-0 group-hover:max-w-full transition-all duration-500 h-0.5 bg-sky-400"></span>
                            </span>
      ...
      <Transition.Root show={Boolean(id)} as={Fragment}>
        <Dialog as="div" onClose={() => setLocation(`/${Selection.beacon_bad_block}`)}>
          <Transition.Child
            as={Fragment}
                        ...
                        <div className="flex items-start justify-between">
                          ...
                          <div className="ml-3 flex h-7 items-center">
                            <button
                              ...
                              onClick={() => setLocation(`/${Selection.beacon_bad_block}`)}
                            >
